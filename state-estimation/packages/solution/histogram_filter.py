...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    dphi_right = 2 * np.pi * right_encoder_ticks / robot_spec["encoder_resolution"]

    # Evaluate bot angular displacement (applies uniformely to all Phi state)
    phi_bot = robot_spec["wheel_radius"] * ( dphi_right - dphi_left) / ( 2 * robot_spec["wheel_radius"] ) # scalar
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                #i_new = i + int(d_lateral[i,j] / grid_spec["delta_d"]) # array
                i_new = i + int(d_lateral / grid_spec["delta_d"]) # scalar
                j_new = j + int(phi_bot / grid_spec["delta_phi"])
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        meas_likelihood_norm = measurement_likelihood / np.sum(measurement_likelihood)
        # Sum of prior belief
        belief = (belief * meas_likelihood_norm ) / np.linalg.norm(meas_likelihood_norm)
